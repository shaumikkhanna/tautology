...
declare
  ...
  shuffled_deck jsonb;
  player_hand jsonb;
  starting_palette_card jsonb;
  turn_ids uuid[] := '{}';
  seated_count integer;
  palettes_dealt integer;
...
begin
  ...
  loop
    ...
    starting_palette_card := shuffled_deck -> 0;
    shuffled_deck := shuffled_deck - 0;

    if starting_palette_card is null then
      raise exception 'Could not deal a starting Palette card.';
    end if;

    update public.red7_players
    set palette = jsonb_build_array(starting_palette_card),
      eliminated = false
    where id = player_row.id;
  ...
  select count(*) into palettes_dealt
  from public.red7_players
  where room_id = room_row.id
    and active
    and role = 'seated'
    and jsonb_array_length(palette) = 1;

  if palettes_dealt <> seated_count then
    raise exception 'Not every seated player received a starting Palette card.';
  end if;
